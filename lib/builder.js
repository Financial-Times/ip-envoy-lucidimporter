...
module.exports.dbBuilder = {
  ...
  async haveJourney(journey) {
    ...
    try {
      ...
      if ((journeyConfig.controlGroup > 1) || (journeyConfig.controlGroup < 0)) {
  ...
  async make(lucidColletion, knexConnection) {
    ...
    if (lucidColletion.channel) {
      logger.error('*** This chart contains channels, which are no longer supported ***');
      logger.error('*** Please change these object to type "Action" ***');
    }
    ...
    if (lucidColletion.action) {
      ...
      logger.warn('*** Importing a journey that has no actions ***');
