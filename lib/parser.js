...
module.exports.preParser = {
  ...
  async prepare(knexConnection) {
    ...
    const journeyNames = lc.trackcontainer.map((journey) => journey.name);
    const res = await knexConnectionAsync.select('*').from('core.journey').whereIn('name', journeyNames);
    const existingJourneyNames = res.map((dbJourney) => dbJourney.name);

    lc.trackcontainer = lc.trackcontainer.filter((journeyNameIn) => !existingJourneyNames
      .some((existingJourneyName) => existingJourneyName === journeyNameIn.name));

    if (!lc.trackcontainer.length) {
      logger.info('No new journeys to import');
      return false;
    }
    ...
    const classObjects = ['Boundary', 'Silo', 'RuleSet', 'Action', 'Channel'];
